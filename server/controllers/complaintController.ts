...
export const submitComplaint = async (req: any, res: any) => {
  ...
  try {
    const complainantResult = await db.prepare(`
    ...
    if (req.files && Array.isArray(req.files)) {
      ...
      for (const file of req.files) {
        await insertAttachment.run(complaintId, file.originalname, `/uploads/${file.filename}`);
      }
    }
    
    const staff = await db.prepare("SELECT id FROM users WHERE role = 'DIRECTOR' OR role = 'TEAM_LEADER'").all() as any[];
...
export const trackComplaint = async (req: any, res: any) => {
  const complaint = await db.prepare(`
    ...
    LEFT JOIN tax_centers tc ON c.TAX_CENTER = CAST(tc.id AS TEXT)
    LEFT JOIN categories cat ON c.COMPLAINTS_CATEGORY = CAST(cat.id AS TEXT)
    LEFT JOIN categories subcat ON c.COMPLAINTS_SUB_CATEGORY = CAST(subcat.id AS TEXT)
...
export const updateComplaint = async (req: any, res: any) => {
  ...
  if (updates.length > 0) {
    ...
    const complaint = await db.prepare("SELECT * FROM complaints_case WHERE COMPLAINTS_ID = ? OR COMPLAINT_CODE = ?").get(req.params.id, req.params.id) as any;
...
export const deleteComplaint = async (req: any, res: any) => {
  await db.prepare("DELETE FROM complaints_case WHERE COMPLAINTS_ID = ? OR COMPLAINT_CODE = ?").run(req.params.id, req.params.id);
  res.json({ success: true });
};

export const addResponse = async (req: any, res: any) => {
  const { complaint_id, user_id, message } = req.body;
  await db.prepare("INSERT INTO responses (complaint_id, user_id, message) VALUES (?, ?, ?)").run(complaint_id, user_id, message);
  
  const complaint = await db.prepare("SELECT * FROM complaints_case WHERE COMPLAINTS_ID = ?").get(complaint_id) as any;
  const user = await db.prepare("SELECT * FROM users WHERE id = ?").get(user_id) as any;

  if (user && user.role === 'OFFICER') {
    const leaders = await db.prepare("SELECT id FROM users WHERE role = 'TEAM_LEADER'").all() as any[];
...
export const addAssessment = async (req: any, res: any) => {
  ...
  await db.prepare("INSERT INTO assessments (complaint_id, user_id, findings, recommendation) VALUES (?, ?, ?, ?)").run(complaint_id, user_id, findings, recommendation);
  
  // Update complaint status
  await db.prepare("UPDATE complaints_case SET CASE_STATUS = 'ASSESSED' WHERE COMPLAINTS_ID = ?").run(complaint_id);
...
export const submitFeedback = async (req: any, res: any) => {
  ...
  const complaint = await db.prepare("SELECT * FROM complaints_case WHERE COMPLAINT_CODE = ?").get(tracking_code) as any;
  
  if (!complaint) {
    return res.status(404).json({ error: "Complaint not found" });
  }

  await db.prepare("INSERT INTO responses (complaint_id, user_id, message) VALUES (?, ?, ?)").run(complaint.COMPLAINTS_ID, null, message);
...
export const addAttachments = async (req: any, res: any) => {
  ...
  const complaint = await db.prepare("SELECT COMPLAINTS_ID FROM complaints_case WHERE COMPLAINT_CODE = ?").get(code) as any;
  ...
  try {
    if (req.files && Array.isArray(req.files)) {
      ...
      for (const file of req.files) {
        await insertAttachment.run(complaint.COMPLAINTS_ID, file.originalname, `/uploads/${file.filename}`);
      }

      // Notify assigned officer
      const c = await db.prepare("SELECT RELEVANT_OFFICER, COMPLAINT_CODE FROM complaints_case WHERE COMPLAINTS_ID = ?").get(complaint.COMPLAINTS_ID) as any;
