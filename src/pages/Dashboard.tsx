...
export function Dashboard() {
  ...
  const getCards = () => {
    const baseCards = [
      { title: 'Total Complaints', value: stats.total, icon: <FileText size={24} />, color: 'bg-zinc-950', trend: '+12%', up: true },
      { title: 'Pending Review', value: stats.pending, icon: <Clock size={24} />, color: 'bg-amber-500', trend: '-5%', up: false },
      { title: 'In Progress', value: stats.in_progress, icon: <TrendingUp size={24} />, color: 'bg-blue-500', trend: '+8%', up: true },
      { title: 'Resolved Cases', value: stats.closed, icon: <CheckCircle2 size={24} />, color: 'bg-blue-600', trend: '+15%', up: true },
    ];

    if (user?.role === UserRole.OFFICER) {
      return [
        { title: 'My Total Cases', value: stats.total, icon: <ClipboardList size={24} />, color: 'bg-zinc-950', trend: 'Active', up: true },
        { title: 'My Pending', value: stats.pending, icon: <Clock size={24} />, color: 'bg-amber-500', trend: 'Priority', up: false },
        { title: 'My Resolved', value: stats.closed, icon: <CheckCircle2 size={24} />, color: 'bg-blue-600', trend: '+2', up: true },
    ...
    if (user?.role === UserRole.TEAM_LEADER) {
      return [
        { title: 'Team Total', value: stats.total, icon: <Users size={24} />, color: 'bg-zinc-950', trend: 'Center', up: true },
        { title: 'Unassigned', value: stats.pending, icon: <AlertCircle size={24} />, color: 'bg-red-500', trend: 'Action Required', up: false },
        { title: 'Team Resolved', value: stats.closed, icon: <CheckCircle2 size={24} />, color: 'bg-blue-600', trend: '+10%', up: true },
        { title: 'Efficiency', value: '88%', icon: <Zap size={24} />, color: 'bg-blue-500', trend: '+5%', up: true },
      ];
    }
    ...
    <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-sm p-8 md:p-12 min-h-full">
      <div className="space-y-12">
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-bold text-zinc-900 tracking-tight italic serif">Welcome back, {user?.name.split(' ')[0]}</h1>
          ...
          {user?.tax_center_name && (
            <div className="px-4 py-2 bg-zinc-100 rounded-xl text-xs font-bold text-zinc-600 uppercase tracking-widest border border-zinc-200">
              {user.tax_center_name}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {cards.map((card, i) => (
            <motion.div
              ...
              className="bg-zinc-50 p-6 rounded-3xl border border-zinc-100 hover:border-blue-200 hover:shadow-md transition-all group"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className={cn(
                  ...
                  card.up ? "bg-blue-100 text-blue-600" : "bg-red-100 text-red-600"
                )}>
              ...
              <p className="text-3xl font-bold text-zinc-900 tracking-tight">{card.value}</p>
            </motion.div>
          ))}
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-zinc-50 rounded-3xl border border-zinc-100 p-8">
            ...
            <div className="space-y-6">
              {[1, 2, 3, 4].map((_, i) => (
                <div key={i} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-white hover:shadow-sm transition-all cursor-pointer group border border-transparent hover:border-zinc-100">
                  <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center text-zinc-400 group-hover:bg-zinc-50 transition-all border border-zinc-100">
                    <AlertCircle size={24} />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-zinc-500">Tax Assessment issue by John Doe</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-bold text-zinc-900">2m ago</p>
          ...
          <div className="bg-zinc-950 rounded-3xl p-8 text-white shadow-xl shadow-zinc-200">
            ...
            <div className="space-y-8">
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-zinc-400">Resolution Speed</span>
