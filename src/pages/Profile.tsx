  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-bold text-zinc-900 tracking-tight italic serif">My Profile</h1>
          <p className="text-zinc-500 mt-2">Manage your account settings and security.</p>
        </div>
          ...
          {!isEditing && !isChangingPassword && (
            <>
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white border border-zinc-200 rounded-xl text-sm font-bold text-zinc-600 hover:bg-zinc-50 transition-all"
              >
                <Edit3 size={18} /> Edit Profile
              </button>
              <button 
                onClick={() => setIsChangingPassword(true)}
                className="flex items-center gap-2 px-4 py-2 bg-zinc-950 text-white rounded-xl text-sm font-bold hover:bg-zinc-800 transition-all"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1">
          <div className="bg-white rounded-3xl p-8 border border-zinc-100 shadow-sm text-center">
            <div className="w-24 h-24 bg-zinc-950 rounded-full mx-auto flex items-center justify-center text-3xl font-bold text-white shadow-xl shadow-zinc-200 mb-6">
              {user.name.charAt(0)}
            </div>
            <h2 className="text-xl font-bold text-zinc-900">{user.name}</h2>
            <p className="text-sm text-zinc-500 mt-1">{user.email}</p>
            <div className="mt-6 pt-6 border-t border-zinc-50 flex flex-col gap-3">
              <div className="flex items-center justify-center gap-2 px-3 py-1.5 bg-zinc-50 rounded-full text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                <Shield size={12} />
                {user.role.replace('_', ' ')}
              </div>
            </div>
              ...
              <form onSubmit={handleUpdateProfile} className="space-y-6">
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button 
                    type="submit"
                    disabled={isLoading}
                    className="flex-1 py-3 bg-zinc-950 text-white rounded-xl font-bold hover:bg-zinc-800 transition-all flex items-center justify-center gap-2"
                  >
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-1">
                    <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Full Name</p>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-zinc-900 font-medium">{user.role.replace('_', ' ')}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Tax Center ID</p>
                    <p className="text-zinc-900 font-medium">{user.tax_center_id || 'N/A'}</p>
                  </div>
                </div>
              </div>

              <div className="pt-8 border-t border-zinc-50">
                <h2 className="text-xl font-bold text-zinc-900 mb-6">Security Status</h2>
                <div className="flex items-center gap-4 p-4 bg-zinc-50 rounded-2xl border border-zinc-100">
                  <div className="w-10 h-10 bg-emerald-100 rounded-xl flex items-center justify-center text-emerald-600">
                    <Shield size={20} />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-zinc-900">Account is Protected</p>
                    <p className="text-xs text-zinc-500">Your account is active and secured with standard encryption.</p>
                  </div>
                </div>
              </div>
            </div>
