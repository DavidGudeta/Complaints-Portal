...
export function AssessmentModal({ isOpen, onClose, onSuccess, complaintId, userId }: AssessmentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Add the assessment as a response
      const res = await fetch('/api/internal/responses', {
        ...
        body: JSON.stringify({
          ...
          user_id: userId,
          message: `[ASSESSMENT REPORT]: ${message}`
        })
      });

      if (!res.ok) throw new Error('Failed to save assessment');

      // 2. Update status to ASSESSED
      await fetch(`/api/internal/complaints/${complaintId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: ComplaintStatus.ASSESSED })
      });
      ...
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl border border-zinc-100 overflow-hidden">
        <div className="px-8 py-6 border-b border-zinc-50 flex items-center justify-between bg-zinc-50/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-zinc-900 italic serif">Add Assessment</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Assessment Findings</label>
            <textarea 
              required
              rows={6}
            ...
            />
            <p className="text-[10px] text-zinc-400 font-medium italic">This assessment will be recorded in the case history and the status will be updated to 'ASSESSED'.</p>
          </div>
